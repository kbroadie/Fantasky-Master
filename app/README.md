...
| Tab | What's on it |
|---|---|
| **Standings** | The Show and League leaders side by side: name, points and how far clear they are. Then one table with every player's rank and movement and their Show and League points. For Series 22, behind each row is the contestant they picked this week: their face from the cast group photo, every head the same size with the eyes centred in the row (warmed gold if the pick won), drifting slightly as you scroll. Opening a row uncovers the rest of the face. Series 21 has no group photo in the app, so its rows are plain. Tap **Show** or **League** to sort (▼/▲ shows the direction). Tap a player (the chevron) to open their ten weekly picks. |
