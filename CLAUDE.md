...
- **Standings:**
  ...
  - **Series with cast photos (21 and 22):** the player's pick that week is the row's backdrop (`pickBackdrop` in `table.js`), behind the whole row card.
    ...
    - Frosted glass by rank on the active board (`frost` in `table.js`, `--blur` on `.pc-bg`): the leader's photo is lightly frosted (3px blur), each place down a little more, to 9px for last. Colour is untouched (the earlier colour grading was replaced on request). Opening a row clears its blur (`.pc.open .pc-bg`, a .35s transition). The reshade's dark sides are carried 32px past the row by a box-shadow so the blur doesn't fade them into a bright rim. It follows the sort.
