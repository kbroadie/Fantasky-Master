...
- **Episodes / Cast:** a tab strip over scroll-snapped slides.
  ...
  - Each scored episode ends with **The race so far** (`raceChart` in `episodes.js`), in place of the episode analysis (removed on request; the CSV's `analysis` column isn't shown). It plots each contestant's **gap to the leader** after every episode from 1 to this one (the x axis starts at episode 1): the leader runs flat along 0 at the top (a dashed gold gridline), everyone else below, so the margins stay readable all series (running totals bunched up; tried and replaced on request, as was a rank bump chart). Tidy negative steps from `niceStep`, at most five gridlines, with no y-axis labels (removed on request; the end labels give each gap). One 2px line per contestant in their colour, smoothed with a monotone cubic Bézier (`smooth`, Fritsch–Carlson), which passes through every point and never overshoots one. Dots ringed in the card colour, this episode's number in gold. Each line ends with the first three letters of the name (DM Mono caps in their colour, like the task table's column heads) and gap ("ARM −4"; the leader's total instead); end labels stay at least 15 apart (pushed down, the lowest capped just into the axis margin, then up only where needed), with a faint hairline to the line when moved.
