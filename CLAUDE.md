...
- **Standings:**
  ...
  - Each row shows rank and movement, the player's name (no 👑/🏆 on it), Show, League and a chevron. Tap a row to open that player's ten picks: portraits and points only, with no text line under them. A row has no pressed state (no :active tint); it flashed the row's colour on iOS as it opened. The chevron flipping is the feedback.
  - **Series with cast photos (21 and 22):** the player's pick that week is the row's backdrop (`pickBackdrop` in `table.js`), behind the whole row card.
    ...
    - Frosted glass: every row's backdrop has the same 6px blur (colour untouched; rank grading was removed on request), and opening a row clears it straight away as the row expands (`.pc.open .pc-bg`, a .35s transition; a delay until the expand finished was tried and dropped on request). No edge glow: `.pc-bg` bleeds `--bleed` (24px) past every side of the row, which clips it, so the blur never samples a transparent edge. Positions inside are measured from the bled box (the row's top is at `--bleed`; the reshade's stops are placed on the row via `--r`). The leader's row has no gold edge bar either.
...
- **Episodes / Cast:** a tab strip over scroll-snapped slides.
  ...
  - Each scored episode ends with **The race so far** (`raceChart` in `episodes.js`), in place of the episode analysis (removed on request; the CSV's `analysis` column isn't shown). It plots each contestant's **gap to the leader** after every episode from 1 to this one. The x axis always runs 1 to 10 and the plot and gridlines fill the card: episode 1 sits at the left edge and the race builds to the right week by week, later episodes' numbers faint; only when the current dots would leave too little room for their labels (episodes 9 and 10) does the axis tighten just enough to keep the labels to the right: the leader runs flat along 0 at the top (no gridline at 0; the dashed one was removed on request), everyone else below, so the margins stay readable all series (running totals bunched up; tried and replaced on request, as was a rank bump chart). Tidy negative steps from `niceStep`, at most five gridlines, with no y-axis labels (removed on request; the end labels give each gap). One 2px line per contestant in their colour, smoothed with a monotone cubic Bézier (`smooth`, Fritsch–Carlson), which passes through every point and never overshoots one. No dots on the lines (removed on request; Episode 1, with no lines yet, keeps its dots), this episode's number in gold. Each line ends with the first three letters of the name (DM Mono caps in their colour, like the task table's column heads) and gap ("ARM −4"; the leader's total instead); end labels stay at least 15 apart (pushed down, the lowest capped at the plot's bottom, then up only where needed), with a faint hairline to the line when moved, and a halo in the card colour so gridlines pass behind them.
    - **Tap to follow:** each contestant is a `g[data-who]`; tapping their line (a wide invisible `.rc-tap` path), name or a point brings them forward (moved last in the SVG) and fades the rest (`.race.focus`); a point also reads out that week: "Ep 2 · Armando · 26 points · 15 behind Joanna (3rd)". Tapping the same line again or empty chart shows everyone. No instruction text: the caption is empty (and hidden) until a point is tapped, as is the heat strip's (removed on request).
  ...
  - The Cast header follows the episode head's pattern (centred): the framed portrait (104px), a mono kicker "1st of 5 · Series 22", the full name as a Bungee title, and one mono line "70 points · 17.5 an episode · 1 win" (the total in their colour). Whoever is in first place (rank 1, ties included) gets the winner's gold light: the header is an `.fx-stage` and the portrait a `.pod-col.win`, so `mountPodiumFx` runs the same canvas effect across the whole header. The Cast tab has no "picked N times by the league" line (removed on request). It shows points per episode as bars on one scale for all five contestants (a crown and gold number for a win, no outline). Each bar is exactly its score over the series' best episode (`--f` × `--plot`); the number and crown sit above the plot and never squeeze the bar. A dashed line marks the series median (every contestant's scored episodes), keyed in the card head as "median 16". Then a Performance radar with three axes, Prize, Filmed and Live, each labelled with its line icon (team tasks aren't counted, not even in Filmed).
...
- **Footer** (`footer` in `ui.js`, `#foot` after `<main>`, the same on every tab): a row of seven rubber ducks as the closing rule (one SVG `<symbol>`, gold, 18×14px, with gold hairlines fading out to each side), and nothing else (no text, on request). `fit` in `main.js` leaves room for it, so a short slide ends with the footer at the bottom of the screen.
