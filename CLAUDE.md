...
- **Episodes / Cast:** a tab strip over scroll-snapped slides.
  ...
  - Each scored episode ends with **The race so far** (`raceChart` in `episodes.js`), in place of the episode analysis (removed on request; the CSV's `analysis` column isn't shown). A bump chart: every contestant's rank by running total after each episode from 1 to this one (1st at the top, competition ranking so ties share a rank), one 2px line per contestant in their colour, dots ringed in the card colour, recessive rank gridlines and axis labels, this episode's number in gold. Each line ends with the name (Fredoka, their colour) and total; tied points are nudged 6 apart and end labels kept at least 15 apart. Tap a point (12px hit circles) and the caption reads "Ep 2 · Chloe · 3rd on 27 points".
