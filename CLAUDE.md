...
- **Episodes / Cast:** a tab strip over scroll-snapped slides.
  ...
  - Each scored episode ends with **The race so far** (`raceChart` in `episodes.js`), in place of the episode analysis (removed on request; the CSV's `analysis` column isn't shown). Every contestant's running total after each episode from 1 to this one (the x axis starts at episode 1): the y axis is total points (0-based, tidy steps from `niceStep`, at most five gridlines), one 2px line per contestant in their colour, smoothed with a monotone cubic Bézier (`smooth`, Fritsch–Carlson) so it passes through every point and never dips between episodes. Dots ringed in the card colour, this episode's number in gold. Each line ends with the name (Fredoka, their colour) and total; end labels are kept at least 15 apart inside the plot, with a faint hairline to the line when moved. Tap a point (12px hit circles) and the caption reads "Ep 2 · Chloe · 27 points (3rd)". Late in a series the totals bunch together; that's the honest 0-based view.
