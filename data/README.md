...
|---|---|---|
| `contestant` | contestant per series | `series`, `contestant` (short name), `full_name`, `accent_color` (hex), `portrait_url` (direct `https://i.imgur.com/<id>.png` link), `bio` (who they are, in a sentence or two; no performance), `stat` (how they're doing) |
...
4. **Optional.** Refresh the contestants' `stat` text. `bio` is only who they are, so it rarely changes.
